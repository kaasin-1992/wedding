...
function hashStr(s){ let h=5381; s=String(s); for(let i=0;i<s.length;i++) h=((h<<5)+h+s.charCodeAt(i))>>>0; return 'h'+h.toString(36); }
function isWeed(text){ return /(?<![а-яіїєґ])трав(а|и|у|і|о|к|ичк|ʼ|')|weed|🌿|канабіс|каннабіс|косяк|джойнт|шишк/i.test(String(text||'')); }
...
// лишатись у пам'яті/localStorage гостя повністю — раніше лише рендер ховав бюджет/травку,
...
function stripGuestData(state){
  return {
    budget: [],
    tasks: (state.tasks||[]).filter(t=>!isWeed(t.text)),
    scriptSofia: (state.scriptSofia||[]).filter(m=>!isWeed(m.text)),
    scriptWave: (state.scriptWave||[]).filter(m=>!isWeed(m.text)),
    weedPassHash: '',
  };
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = { today, daysUntil, daysUntilWedding, getStatus, plDays, hashStr, isWeed, timeMask, timeNorm, stripGuestData };
}
