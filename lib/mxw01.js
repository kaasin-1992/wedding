...
/* Режими друку. 0 — один біт на піксель, це наш випадок: дизеринг ми вже
   зробили самі й краще, ніж це зробить принтер. */
...
/* Розбір відповіді на A1. Індекси — з опису протоколу; він сам зізнається,
   що неповний, тому все, чого може не вистачити в короткому пакеті,
   віддаємо як null, а не як нуль: «невідомо» і «нуль» — різні речі,
   і показувати «0% батареї» замість «невідомо» було б брехнею. */
function mxParseStatus(payload) {
  if (!payload || payload.length < 7) return null;
  const at = i => (payload.length > i ? payload[i] : null);
  const flag = at(12);
...
function mxDataChunks(data, chunkSize) {
  const size = chunkSize || 180;
  const chunks = [];
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    MX_CMD, MX_MODE_1BPP, MX_DEF_INTENSITY,
